...
public abstract class Module extends Feature implements Jsonable {
    ...
    // List to keep track of enabled modules
    ...
    public void enable() {
        // Add module to enabledModules list
        ...
        if (OyVey.moduleManager.getModuleByClass(Notifications.class).isEnabled()) {
            Command.sendMessage(Formatting.GRAY + this.getDisplayName() + " was " + Formatting.GREEN + "Enabled");
        }
    ...
    public void disable() {
        // Remove module from enabledModules list
        enabledModules.remove(this);
        if (this.hasListener && !this.alwaysListening) {
            EVENT_BUS.unregister(this);
        }
        if (OyVey.moduleManager.getModuleByClass(Notifications.class).isEnabled()) {
            Command.sendMessage(Formatting.GRAY + this.getDisplayName() + " was " + Formatting.RED + "Disabled");
        }
    ...
    public String getDisplayName() {
        return this.displayName.getValue() +Formatting.GRAY+"["+Formatting.WHITE+this.getDisplayInfo()+Formatting.GRAY+"]";
    }
    ...
    public abstract void onRender2D(me.alpha432.oyvey.features.impl.Render2DEvent event);
